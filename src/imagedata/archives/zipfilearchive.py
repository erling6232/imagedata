...
from abc import ABC
# from typing import Any, Dict, IO, KeysView, Literal, Optional, Type
from typing import Any, Dict, IO, KeysView, Optional, Type
try:
...
class WriteFileIO(io.FileIO):
    ...
    __archive: zipfile.ZipFile
    __filename: str
    __local_file: IO[bytes]

    def __init__(self, archive: zipfile.ZipFile, member: Member, local_file: IO[bytes]):
...
class ZipfileArchive(AbstractArchive, ABC):
    ...
    # Internal data
    __archive: zipfile.ZipFile  # ZipFile object
    __transport: AbstractTransport  # file transport object
    __fp: IO[bytes]  # zip file object in transport object
    __mode: Literal["r", "w", "x", "a"]  # read or write access
    __files: Dict[str, Member]  # dict of members in the zip archive
    __path: str  # path to the zip file using given transport
    ...
    __tmpdir: str  # local directory where zip file is unpacked
    ...
    @staticmethod
    def _longest_prefix(keys: KeysView, required: str) -> str:
        prefix = ''
        for folder in keys:
    ...
    def _filehandle_in_files(self, filehandle: Member) -> bool:
