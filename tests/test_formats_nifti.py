...
class Test3DNIfTIPlugin(unittest.TestCase):
    ...
    def test_read_2D(self):
        ...
        #    'data/nifti/time_all/time_all_fl3d_dynamic_20190207140517_14.nii.gz')
        #self.assertEqual(si1.shape, (10, 40, 192, 152))
        d = Series('data/dicom/time/time00/')
        #si2 = si1[0,0,...]
        si2 = d[0,...]
        si2.write('ttn3d', formats=['nifti'])
        si3 = Series('ttn3d', template=si2)
        si3.write('ttn3dd', formats=['dicom'])
    ...
    def test_read_3D(self):
        d = Series('data/dicom/time/time00/')
        d.write('ttn3d', formats=['nifti'])
        si3 = Series('ttn3d', template=d)
        si3.write('ttn3dd', formats=['dicom'])
        self.assertEqual(d.dtype, si3.dtype)
        self.assertEqual(d.shape, si3.shape)
        np.testing.assert_array_equal(d, si3)
    ...
    def test_qform_3D(self):
        ...
        d.write('ttn3d', formats=['nifti'])
        n = Series('ttn3d')
        M = n.transformationMatrix
        self.assertEqual(d.shape, n.shape)
        self.assertEqual(d.dtype, n.dtype)
        np.testing.assert_array_almost_equal(d.transformationMatrix, n.transformationMatrix, decimal=2)
    ...
    def test_compare_qform_to_dicom(self):
        ...
        self.assertEqual(d.shape, n.shape)
        #self.assertEqual(d.dtype, n.dtype)
        np.testing.assert_array_almost_equal(d.transformationMatrix, n.transformationMatrix)
    ...
    def test_read_3d_nifti_no_opt(self):
        si1 = Series('data/nifti/time_all/time_all_fl3d_dynamic_20190207140517_14.nii.gz')
...
class Test4DNIfTIPlugin(unittest.TestCase):
    ...
    def test_write_4d_nifti(self):
        ...
        si1.write('ttn4d', formats=['nifti'], opts=self.opts)
