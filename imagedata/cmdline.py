...
def add_argparse_options(parser):
    parser.add_argument('--of', dest="output_format", action=OutputFormatAction,
        help="Output format [dicom|itk|nifti|biff|mat|ps] (default: dicom). Replaces %%p in output path.",
...
if __name__ == '__main__':
    ...
    try:
        si = imagedata.imagedata(args.in_dirs, args.input_order, args)
    except UnknownInputError:
        print("Could not determine input format of %s." % args.in_dirs[0])
        sys.exit(1)

    #tag,fname,im = hdr.DicomHeaderDict[0][0]
    #image_type = im.ImageType

    write_images(hdr, si, os.path.join(args.out_name, "Image_%05d"), args)
