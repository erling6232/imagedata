...
def copy_general_dicom_attributes(ds_in, ds_out):
    ...
    if 'PixelSpacing' in ds_in: ds_out.PixelSpacing = ds_in.PixelSpacing
    if 'ImageOrientationPatient' in ds_in: ds_out.ImageOrientationPatient = ds_in.ImageOrientationPatient
    if 'ImagePositionPatient' in ds_in: ds_out.ImagePositionPatient = ds_in.ImagePositionPatient
    ...
    if 'SamplesPerPixel' in ds_in: ds_out.SamplesPerPixel = ds_in.SamplesPerPixel
    if 'PhotometricInterpretation' in ds_in: ds_out.PhotometricInterpretation = ds_in.PhotometricInterpretation
    if 'Rows' in ds_in: ds_out.Rows = ds_in.Rows
    if 'Columns' in ds_in: ds_out.Columns = ds_in.Columns
    if 'BitsAllocated' in ds_in: ds_out.BitsAllocated = ds_in.BitsAllocated
    if 'BitsStored' in ds_in: ds_out.BitsStored = ds_in.BitsStored
    if 'HighBit' in ds_in: ds_out.HighBit = ds_in.HighBit
    if 'PixelRepresentation' in ds_in: ds_out.PixelRepresentation = ds_in.PixelRepresentation
